...
% Top folder level (to be selected by user via uigetfile)
    % Contains 
        % Info.csv a 3 column matrix. No Header
            ...
            % column 3 contains the axial length formated as ##.##
...
mfileshortpath = erase(mfilefullpath, 'OCTA_Metrics_v4p3.m');
...
for kk = 1:length(root_dir)
    ...
    for kkk = 1:size(Current_subfolder,1)
        ...
        Currentloss_pathparts = strsplit(Current_lossfolder{kkk,2},'\');
        ...
        newname = strcat(Currentloss_pathparts{4},'_',Currentloss_pathparts{5},'pct_Raw_Data.csv');
