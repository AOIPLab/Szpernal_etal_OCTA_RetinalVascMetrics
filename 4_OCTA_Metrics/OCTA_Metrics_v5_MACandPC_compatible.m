% OCTA_Metrics_v5_MACandPC_compatible.m
% Created on: 2021?
...
elseif isunix
    error('This script has not been validated for use with Linux operating system. Use at your own risk.')
else
    error('Platform not supported. This script has been validated for use with Mac and PC.')
end

    
mfilefullpath = matlab.desktop.editor.getActiveFilename;
mfileshortpath = erase(mfilefullpath, 'OCTA_Metrics_v5_MACandPC_compatible.m');
...
for kk = 1:length(root_dir)
    ...
    if OperatingSys == 'PC'
        searchlist = strsplit(searchlist, ';'); %PC
    elseif OperatingSys == 'Mac'
        searchlist = strsplit(searchlist, ':'); %Mac
    end
    ...
    for kkk = 1:size(Current_subfolder,1)
        ...
        if OperatingSys == 'PC'
            Currentloss_pathparts = strsplit(Current_lossfolder{kkk,2},'\'); %\ for PC
        elseif OperatingSys == 'Mac'
            Currentloss_pathparts = strsplit(Current_lossfolder{kkk,2},'/'); %/ for MAC
        end
        ...
        newname = strcat(Currentloss_pathparts{length(Currentloss_pathparts)-1},'_',Currentloss_pathparts{length(Currentloss_pathparts)},'pct_Raw_Data.csv'); %Need to change {#} based on folder structure (Mac / PC) 4, 5 for PC and 11, 12 for Mac
