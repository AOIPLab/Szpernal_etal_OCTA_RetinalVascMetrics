...
mfileshortpath = erase(mfilefullpath, 'OCTA_VesselLoss_v4.m'); 
...
pathparts = strsplit(root_path,'\'); %split up file path parts
...
for i = 1:size(skel_dir,1) %For the number of images in the binary directory
    ...
    iter = 2; %Set number of iterations for randomization %1000
    
    for a = 1:length(loss_range) %For each percent loss in the csv file
        pctloss = loss_range(a); %Percent loss
        iniloss = pctloss; %Creating .2% below the expected loss so the actual loss doesn't overshoot the intended loss %pctloss - .002;
        ...
        for b = 1:iter
            ...
            losspic = 255*uint8(loss_with_mask);
            imwrite(losspic,[pctpath '\' subID '_OD_rloss_' num2str(pctloss*100) 'pct_' num2str(b) 'i.bmp']);
        end
    trueloss = trueloss';
    writematrix(trueloss,[pctpath '\' subID '_OD_rloss_' num2str(pctloss*100) 'pct_RealLoss.csv']);
    end
end
toc
