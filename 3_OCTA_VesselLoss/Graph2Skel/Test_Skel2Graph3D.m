...
load JC_0677_optoskel.csv;
skel = JC_0677_optoskel;
skeltemp = zeros(size(skel(:,:,1)));
[~, ~, imdepth] = size(skel);
for i = 1:imdepth; skeltemp = skeltemp + skel(:,:,i);end
skeltemp = bwmorph(skeltemp,'skel');
...
[~,node,link] = Skel2Graph3D(skeltemp,0);
