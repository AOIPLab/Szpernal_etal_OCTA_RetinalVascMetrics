...
groundtruth_sd = std(cell2mat(groundtruth(2:end,2:end)),1);%Calculating standard deviations of the population
...
norm_name = strcat(normname(1:end-12),'Normative_Data.csv'); %Changing name to show z-score calculations
writematrix(NormData(:,:,v), fullfile(RawData_dir{v,2},'\',norm_name)); %Saving z-score matrix to an excel sheet

for m = 1:length(NormData)
...
writematrix(zscore(:,:,v), fullfile(RawData_dir{v,2},'\',zscore_name)); %Saving z-score matrix to an excel sheet

zlogical = zeros(1000,13); %Preallocating the 1st matrix for turning the z-scores that are signficiant to 1 and the rest to 0
for n = 1:13
    for l = 1:1000 
